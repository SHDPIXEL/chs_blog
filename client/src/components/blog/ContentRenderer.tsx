  ...
  useEffect(() => {
    ...
    if (contentRef.current) {
      ...
      allImages.forEach((img, index) => {
        ...
        if (
          imgLink &&
          imgElement.parentElement?.tagName.toLowerCase() !== "a"
        ) {
          console.log(
            `Image ${index + 1} has link attribute but no anchor parent, creating anchor`,
          );
          ...
          if (height) {
            imgElement.style.height = height.includes("px")
              ? height
              : `${height}px`;
          }
          ...
          indicator.className =
            "absolute top-2 right-2 bg-blue-500 text-white px-2 py-1 rounded-md shadow-md text-xs flex items-center";
          indicator.innerHTML =
      ...
      console.log(
        `Found ${imageContainers.length} image containers with data-type="image"`,
      );
